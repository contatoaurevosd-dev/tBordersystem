...
      throw error;
    } finally {
      // Nenhuma liberação aqui, pois pode falhar se não estiver conectado.
    }
  }
...
   * Executa openDevice e claimInterface.
   */
...
    try {
      // A chamada handler.connect() é o que executa openDevice + claimInterface
...
      }
      
      // *** CORREÇÃO CRÍTICA AQUI: Se a conexão falhar (claimInterface), 
      // o deviceHandler PODE ter sido parcialmente inicializado. 
      // É prudente tentar desconectar (que fará o release) para limpar o estado.
      try {
        await handler.disconnect();
      } catch(e) {
        console.warn('Falha na limpeza do handler após erro de permissão/conexão', e);
      }
...
      try {
        // Este é o ponto onde o recurso USB deve ser liberado (release interface)
...
    // O método write envia os dados, mas não gerencia a conexão/desconexão
    await this.deviceHandler.write(data);
  }

  /**
   * NOVO MÉTODO PRINCIPAL: Gerencia o ciclo de vida da impressão.
   * Garante a liberação da interface em caso de falha de impressão,
   * prevenindo o erro 'Unable to claim interface'.
   */
  async executePrintJob(printData: string): Promise<void> {
    if (!this.isConnected()) {
      // Se não estiver conectado, tenta reconectar ou conectar antes de imprimir
      try {
        await this.searchPrinter();
      } catch(e) {
        console.error('Erro ao conectar antes da impressão:', e);
        throw new Error('Não foi possível estabelecer conexão com a impressora.');
      }
    }
    
    if (!this.isConnected()) {
      throw new Error('Impressora não está conectada ou pronta para imprimir.');
    }

    try {
      // Envia os dados para a impressora (write)
      await this.sendData(printData);
    } catch(e) {
      console.error('Erro durante o envio dos dados de impressão:', e);
      // *** CRÍTICO: Tentativa de liberação do recurso após falha de envio.
      // Isso pode resolver o "claimInterface" se a falha do write for o que está
      // deixando a interface travada.
      try {
        console.log('Tentando desconectar/liberar após erro de escrita...');
        await this.deviceHandler?.disconnect(); 
      } catch(e2) {
        console.warn('Falha na desconexão de emergência:', e2);
      }
      throw new Error('Falha no envio dos dados para a impressora.');
    } finally {
      // Se a conexão for feita a cada trabalho (e não persistente), 
      // a liberação final deve ocorrer aqui. 
      // Manteremos persistente para o "Print Bridge" e faremos apenas a 
      // desconexão de emergência acima.
    }
  }
...
    // Usa o novo método para garantir a liberação em caso de falha
    await this.executePrintJob(testData);
  }
...
    // ... (Restante da lógica de formatação do texto) ...
    // (mantive o restante da função igual para brevidade)
...
    // Usa o novo método para garantir a liberação em caso de falha
    await this.executePrintJob(parts.join(''));
  }
